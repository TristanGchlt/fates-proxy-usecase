...
# Date   : 2026-07-13T16:05:03.797785400Z
...
@jpipe_link("deployable:fair:di_threshold")
@jpipe(produce=[], consume=['model', 'model_type', 'test_data'])
def disparate_impact_is_greater_than_0_8(model, 
                                         model_type, 
                                         test_data, 
                                         produce: JpipeProduce) -> bool:
    """[strategy] Disparate Impact is greater than 0.8"""
    from src.metrics.fairness import disparate_impact
    from src.models.utils import predict
    test_data["y_pred"] = predict(model, model_type, test_data["X_test"])
    di = disparate_impact(test_data)
    if di <= 0.8 :
        return False
    return True
...
@jpipe(produce=['model', 'model_type'])
...
@jpipe(produce=[], consume=['model', 'model_type', 'test_data'])
def accuracy_is_greater_than_0_8(model, 
                                 model_type, 
                                 test_data, 
                                 produce: JpipeProduce) -> bool:
